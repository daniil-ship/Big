...
# Windows 11 UTF-8 console output setup
...
class PEBuilder:
    ...
    def build(self) -> bytes:
        ...
        # Keep offsets
        import_names = [b"GetStdHandle", b"WriteFile", b"ExitProcess", b"SetConsoleOutputCP", b"SetConsoleCP"]
        hint_offsets = {}
        ...
        # INT array: N qwords + null terminator
        int_rvas=[]
        ...
        # IAT array same size
        for _ in range(len(import_names) + 1):
            rdata.extend(b"\x00"*8)
        ...
        # ... leave zero
        # 12 IAT
        iat_size = (len(import_names) + 1) * 8
        struct.pack_into("<I", pe, opt_off+112+8*12, iat_rva)
        struct.pack_into("<I", pe, opt_off+112+8*12+4, iat_size)
...
""")
    ...
    def generate_bytes(prog, target):
        ...
        if target=="windows":
            ...
            iat_base2=iat_rva2
            iat_map2={
                "GetStdHandle": iat_base2 + 0,
                "WriteFile": iat_base2 + 8,
                "ExitProcess": iat_base2 + 16,
                "SetConsoleOutputCP": iat_base2 + 24,
                "SetConsoleCP": iat_base2 + 32,
            }
            # locals
            var_names2=[]
            for stmt in main_func.body.stmts:
            ...
            # Set console codepage to UTF-8 (65001) for Windows 11 console/PowerShell
            em2.emit(0x48, 0x83, 0xEC, 0x20) # sub rsp, 32
            em2.emit(0xB9, 0xE9, 0xFD, 0x00, 0x00) # mov ecx, 65001
            em2.mov_reg_imm64("rax", IMAGE_BASE + iat_map2["SetConsoleOutputCP"])
            em2.emit(0xFF, 0x10) # call [rax]
            em2.emit(0xB9, 0xE9, 0xFD, 0x00, 0x00) # mov ecx, 65001
            em2.mov_reg_imm64("rax", IMAGE_BASE + iat_map2["SetConsoleCP"])
            em2.emit(0xFF, 0x10) # call [rax]
            em2.emit(0x48, 0x83, 0xC4, 0x20) # add rsp, 32
            ...
            def emit_print_win(raw):
                ...
                str_abs=IMAGE_BASE+RDATA_RVA+off
                str_len=len(raw.encode("utf-8"))
                # 64 bytes: 16-byte aligned before call
                em2.emit(0x48,0x83,0xEC,0x40)
    ...
    sys.stderr.write(f"  = note: запусти {out_path}\n")
    sys.exit(0)
